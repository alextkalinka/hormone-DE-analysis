...
The low-expression `filter_cutoff` is set to 0 because a filter of 40 was applied when the batch-correction was pre-computed.
...
```{r}
kable(delboy::hits(dboy.day35))

```

# Appendix

To run the code in the following section, change the `eval=F` option in the chunk header to `eval=T`.
